<template>
  <div v-if="ordersByStatus?.data.length > 0">
    <PageTitle text="Üretim Bekleyen Siparişler" />
    <UretimCard :order="ordersByStatus.data" />
  ...
  <div v-else>
    <div class="mt-10 max-w-7xl h-96 p-4 mx-auto sm:px-6 lg:px-8 bg-blue-50 flex items-center justify-center">
      <p class="text-center text-2xl text-gray-500 ">Üretim Bekleyen Sipariş Bulunmamaktadır.</p>
    </div>
  </div>
...
import UretimCard from '@/components/Desinger/UretimCard.vue';
import Pagination from '@/components/Desinger/Pagination.vue';

import { storeToRefs } from 'pinia'

import { useDesingerProduction } from '@/stores/tasarimci/useUretim.js';
import { onMounted } from 'vue';

const store = useDesingerProduction();
