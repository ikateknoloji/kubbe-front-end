<template>
  <div v-motion-fade class="shadow w-full md:w-3/4 mb-10" role="alert" v-for="notification in tasarimciNotifications"
    :key="notification.id">
    <router-link @click="store.markNotificationAsRead(notification.id)"
      :to="`/dashboard/tasarimci/${notification.message.order.original_status == 'DP' ? 'tasarim-bekleyen' : 'tasarim-guncelle'}/${notification.message.order.id}`">
  ...
  <div v-if="tasarimciNotifications?.length === 0">
    <div class="mt-10 max-w-7xl h-96 p-4 mx-auto sm:px-6 lg:px-8 bg-blue-50 flex items-center justify-center">
      <p class="text-center text-2xl text-gray-500 ">Herhangi Bir Bildiriminiz Bulunmamaktadır.</p>
    </div>
  </div>
</template>
...
const { tasarimciNotifications, pagination } = storeToRefs(store)
