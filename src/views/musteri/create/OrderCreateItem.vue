<template>
  ...
  <CoverContent title="Sipariş Kalemi Oluştur">
          ...
          <Listbox v-model="selectedCategory">
            ...
            <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
              ...
              <ListboxOptions
                ...
                <ListboxOption v-slot="{ active, selected }" v-for="item in productCategories" :key="item.id"
    ...
    <div class="w-full flex-col block pt-5 mt-5 xl:items-center sm:flex xl:flex-row first:mt-0 first:pt-0">
      ...
      <div class="w-full  gap-4 flex mt-4  mb-3">
        <div class="w-full max-w-xs mb-10 relative z-[20]">
  ...
  <CoverContent title="Sipariş Sepeti">
    <div class="overflow-y-auto">
      <div v-if="orderDisplay.length > 0" class=" max-w-6xl mb-5 font-poppins font-medium">
        <div class="relative grid grid-cols-4  mb-4" v-for="(item, i) in orderDisplay" :key="i" :value="item">
...
async function resetValues() {
  await create.addOrderItem(selectedCategory.value, selectedType.value, color.value, quantity.value, unit_price.value, type.value, newColor.value)
