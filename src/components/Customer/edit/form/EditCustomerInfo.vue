  ...
  <div class="flex flex-col p-5 mb-10">
    <div>
      <div class="flex-col block pt-5 mt-5 xl:items-center sm:flex xl:flex-row first:mt-0 first:pt-0">
        <label class="inline-block mb-2 sm:mb-0 sm:mr-5 sm:text-right xl:w-60 xl:mr-14">
          <div class="text-left">
            ...
            <div class="mt-1.5 xl:mt-3 text-xs leading-relaxed text-slate-500/80">
              Lütfen Müşterinin Adve Soyad bilgisini girin.Bu bilgiler gereklidir.
            </div>
          </div>
        </label>
        <div class="max-w-xl flex-1 w-full mt-3 xl:mt-0 mr-4">
        ...
        <div class="max-w-xl flex-1 w-full mt-3 xl:mt-0">
          <div class="flex flex-col items-center md:flex-row">
            <input type="text"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
                focus:ring-blue-500 focus:border-blue-500/40 block w-full p-2.5 outline-none placeholder:text-slate-400" required
      ...
      <div class="flex-col block pt-5 mt-5 xl:items-center sm:flex xl:flex-row first:mt-0 first:pt-0">
        <label class="inline-block mb-2 sm:mb-0 sm:mr-5 sm:text-right xl:w-60 xl:mr-14">
          <div class="text-left">
            <div class="flex items-center">
              <div class="font-medium">Email Adresiniz</div>
              <div
                class="ml-2.5 px-2 py-0.5 bg-slate-100 text-slate-500 dark:bg-darkmode-300 dark:text-slate-400 text-xs rounded-md border border-slate-200">
                Gerekli
              </div>
            ...
            <div class="mt-1.5 xl:mt-3 text-xs leading-relaxed text-slate-500/80">
              Lütfen geçerli bir e-posta adresi girin erişiminiz var.
            </div>
          </div>
        </label>
        <div class="max-w-2xl flex-1 w-full mt-3 xl:mt-0">
          <input type="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
            ...
            required v-model="customerInfo.email" />
        </div>
      </div>
      <div class="flex-col block pt-5 mt-5 xl:items-center sm:flex xl:flex-row first:mt-0 first:pt-0">
        <label class="inline-block mb-2 sm:mb-0 sm:mr-5 sm:text-right xl:w-60 xl:mr-14">
          <div class="text-left">
            <div class="flex items-center">
              <div class="font-medium">Telefon Numarası</div>
              <div
                class="ml-2.5 px-2 py-0.5 bg-slate-100 text-slate-500 dark:bg-darkmode-300 dark:text-slate-400 text-xs rounded-md border border-slate-200">
                İsteğe Bağlı
              </div>
            ...
            <div class="mt-1.5 xl:mt-3 text-xs leading-relaxed text-slate-500/80">
              Lütfen geçerli bir telefon numarası girin erişiminiz var.
            </div>
          </div>
        </label>
        <div class="max-w-2xl flex-1 w-full mt-3 xl:mt-0">
          <input type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
            focus:ring-blue-500 focus:border-blue-500/40 block w-full p-2.5 outline-none placeholder:text-slate-400"
            required v-model="customerInfo.phone" />
        </div>
      </div>
    </div>
...
const customerInfo = ref({
  name: props.data.order.customer_info.name,
  surname: props.data.order.customer_info.surname,
  phone: props.data.order.customer_info.phone,
  email: props.data.order.customer_info.email,
});

const updateCustomerInfo = async (orderId) => {
  try {
    const response = await apiClient.post(`/update-customer-info/${orderId}`, customerInfo.value);
