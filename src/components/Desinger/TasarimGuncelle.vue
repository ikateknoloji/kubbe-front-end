    ...
    <div class="flex items-center justify-center w-full mb-10">
      <label for="dropzone-file"
        ...
        <div v-if="image" class="flex justify-center w-full p-4">
          ...
          <img v-if="fileType.value && fileType.value.startsWith('image/')" :src="image" class="" />
...
const onUpload = async () => {
  ...
  formData.append('design_image', fileInput.value.files[0]);

  try {
    // POST isteğini yapın
    const response = await apiClient.post(`/update-design/${props.orderId}`, formData, {
      headers: {
    ...
    if (response.data.message) {
      ...
      toast(response.data.message, {
        ...
        onClose: () => { // Bildirim kapatıldığında tetiklenir
          router.push(`/dashboard/tasarimci/tasarim-guncelle`); // Yönlendirme işlemini gerçekleştir
        }
      });
