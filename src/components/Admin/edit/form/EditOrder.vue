      ...
      <div class="flex-col block pt-5 mt-5 xl:items-center sm:flex xl:flex-row first:mt-0 first:pt-0">
        <label class="inline-block mb-2 sm:mb-0 sm:mr-5 sm:text-right xl:w-60 xl:mr-14">
          <div class="text-left">
            <div class="flex items-center">
              <div class="font-medium">Sipariş Notunuzu Girin</div>
              <div
            ...
            <div class="mt-1.5 xl:mt-3 text-xs leading-relaxed text-slate-500/80">
              Lütfen geçerli bir sipariş notu girin.
            </div>
          </div>
        </label>
        <div class="max-w-2xl flex-1 w-full mt-3 xl:mt-0">
          <textarea
            ...
            required v-model="orderInfo.note" rows="4"></textarea>
        </div>
      </div>
    </div>
  </div>
...
const orderInfo = ref({
  ...
  note: props.data.order.note ? props.data.order.note : null,
});

const updateCustomerInfo = async (orderId) => {
  try {
    const response = await apiClient.post(`/update-note-info/${orderId}`, orderInfo.value);
