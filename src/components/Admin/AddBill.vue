...
const onUpload = async () => {
  ...
  formData.append('bill_image', fileInput.value.files[0]);

  try {
    // POST isteğini yapın
    const response = await apiClient.post(`/orders/add-bill/${props.orderId}`, formData, {
      headers: {
