<template>
  ...
  <div v-show="isOpen"
    class="z-[999] fixed min-w-fit max-h-screen top-12 md:top-44 left-4 right-4 md:left-1/2 flex items-center justify-center bg-opacity-50  md:-translate-x-1/2">
    <div class="max-w-2xl p-3 bg-white rounded-md shadow-xl">
      ...
      <div class="mt-4">
        <!--Note-->
        <div class="w-full gap-4 mb-10 px-1 md:px-8 mt-10">
          ...
          <div class="w-full max-w-2xl">
            ...
            <div class="mt-10 flex flex-wrap items-center justify-center space-y-4">
              <button @click="isOpen = false"
                class="px-6 py-[9px] text-red-800 border border-red-600 rounded mr-2 i w-full">İptal</button>
              <button type="button" @click="rejectOrder(order.id)"
                class=" w-full py-3 px-8 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-red-500 text-white hover:bg-red-600 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                Ödeme Reddet
              </button>
            </div>
          </div>
